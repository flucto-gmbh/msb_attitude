...
class IMUPoller(Thread):
    ...
    def __init__(self, topic : str, config : dict, context : zmq.Context):
        ...
        self._connect_to = f'{self._config["ipc_protocol"]}:{self._config["subscriber_ipc_port"]}'
        self._zmq_socket = self._zmq_ctx.socket(zmq.SUB)

        try:
            self._zmq_socket.connect(self._connect_to)
        except Exception as e:
            logging.fatal(f'failed to connect to socket {self._zmq_socket}: {e}')

        logging.debug(f'connected to zeroMQ XPUB socket: {self._connect_to}')
    ...
    def __del__(self):
        try:
            ...
            logging.error(f'failed to close zmq socket: {e}')
